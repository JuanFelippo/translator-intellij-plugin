...
import compat.AccessibleContextUtilCompat;
import compat.ScreenReaderCompat;
...
public class Balloon implements com.intellij.openapi.ui.popup.Balloon {
    ...
    private PositionTracker<com.intellij.openapi.ui.popup.Balloon> myTracker;
    ...
    public Balloon(@NotNull JComponent content,
    ...
    @Override
    public void show(final RelativePoint target, final Position position) {
    ...
    @Override
    public void show(PositionTracker<com.intellij.openapi.ui.popup.Balloon> tracker, Position position) {
    ...
    private void show(RelativePoint target, AbstractPosition position) {
        show(new PositionTracker.Static<com.intellij.openapi.ui.popup.Balloon>(target), position);
    }

    private void show(PositionTracker<com.intellij.openapi.ui.popup.Balloon> tracker, AbstractPosition position) {
        ...
        if (mnemonicsFix) {
            ...
            myFocusManager.doWhenFocusSettlesDown(new ExpirableRunnable() {
                ...
                @Override
                public void run() {
                    IdeEventQueue.getInstance().disableInputMethods(Balloon.this);
    ...
    @Override
    public void revalidate(@NotNull PositionTracker<com.intellij.openapi.ui.popup.Balloon> tracker) {
    ...
    private void hideAndDispose(final boolean ok) {
        ...
        if (myTraceDispose) {
            Logger.getInstance("#com.intellij.ui.Balloon").error("Dispose balloon before showing", new Throwable());
        }
        ...
        final Runnable disposeRunnable = new Runnable() {
            ...
            public void run() {
                ...
                for (JBPopupListener each : myListeners) {
                    each.onClosed(new LightweightWindowEvent(Balloon.this, ok));
                }

                Disposer.dispose(Balloon.this);
                onDisposed();
            }
        };
    ...
    private abstract static class AbstractPosition {
        abstract EmptyBorder createBorder(final Balloon balloon);
        ...
        public void updateBounds(final Balloon balloon) {
        ...
        void paintComponent(Balloon balloon, final Rectangle bounds, final Graphics2D g, Point pointTarget) {
    ...
    private static class Below extends AbstractPosition {
        ...
        @Override
        EmptyBorder createBorder(final Balloon balloon) {
        ...
        @Override
        protected Shape getPointingShape(final Rectangle bounds, final Point pointTarget, final Balloon balloon) {
    ...
    private class MyComponent extends JPanel implements ComponentWithMnemonics {
        ...
        private MyComponent(JComponent content, Balloon balloon, EmptyBorder shapeBorder) {
            setOpaque(false);
            setLayout(null);
            myBalloon = balloon;
            ...
            putClientProperty(com.intellij.openapi.ui.popup.Balloon.KEY, Balloon.this);
    ...
    private static class Shaper {
        ...
        public Shaper(Balloon balloon, Rectangle bounds, Point targetPoint, @JdkConstants.TabPlacement int targetSide) {
            myBalloon = balloon;
            myBounds = bounds;
